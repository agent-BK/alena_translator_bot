...
def start_change_language(message):
    ...
    if ActionsChanel().checking_group_member(message_id):
        ...
        bot.register_next_step_handler(message, change_language_input)
    else:
        ActionsChanel().channel_subscription_offers(message)


def change_language_input(message):
    text = message.text
    message_id = message.from_user.id
    ...
    if text in Languages.dict_languages_translation.values():
        ...
        base_lang = translation.base_lang(message, read=True)
        text_out = BotText.language_not_changed.format(
            Languages.dict_languages_translation[base_lang[0]], Languages.dict_languages_translation[base_lang[1]])
...
def change_language_out(message, lang_in_temp):
    text = message.text
    message_id = message.from_user.id
    ...
    if text in Languages.dict_languages_translation.values():
        ...
        text_out = BotText.languages_selected.format(
            Languages.dict_languages_translation[lang_in], Languages.dict_languages_translation[language_out])
        bot.send_message(message_id, text=text_out, reply_markup=config_keyboard.keyboard_remove())
        ...
        bot.send_message(message_id, text=BotText.not_understand_change_lang)
        bot.register_next_step_handler(message, change_language_out, lang_in_temp)
...
def voice_processing(message):
    ...
    if ActionsChanel().checking_group_member(message_id):
        ...
        audio_bytes = conversion.convert(input_bytes=file.content, ogg=True)
